...
void fs_debug_dump(unsigned int type, size_t bytes)
{
	...
	/* do not check read/write records too frequently  */
	if (time_before(jiffies, last_jiffies + HZ))
		return;

	/* check read/write records */
	check_dbg_threshold(&current->ioac, &dbg_threshold[type], type);

	return;
}
/* end of FS debug dump */
		...
		size_t, count)
{
	...
	if (!get_tamper_sf() && file != NULL) {
		/* TODO unify recovery mode -> QCT use MFG_MODE_RECOVERY, NV use BOARD_MFG_MODE_RECOVERY */
		if (board_mfg_mode() != 2
	...
	if (file) {
		...
		/*
		 * debug for system server binder writes
		 * 1. add timeout
		 * 2. show error when vfs_write returns error
		 */
		if (debug_write) {
