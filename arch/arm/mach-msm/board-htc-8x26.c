...
static int __maybe_unused memwl_usb_product_id_match_array[] = {
	0x0ff8, 0x0e70, /* CDC-ECM */
	0x0fa4, 0x0eaf, /* mtp+adb+ums+rmnet */
	0x0fa5, 0x0eb0, /* mtp+ums+rmnet */
	0x0f91, 0x0ec5, /* mtp+ums */
	0x0f64, 0x07d8, /* mtp+ums+acm */
	0x0f63, 0x07d9, /* mtp+adb+ums+acm*/
	0x0f29, 0x07d6, /* mtp+adb+ums+acm+rmnet */
	0x0f2a, 0x07d7, /* mtp+ums+acm+rmnet */
	0x0f9a, 0x0eb2, /* mtp+ums+diag */
	0x0f99, 0x0eb1, /* mtp+ums+adb+diag */
	-1,
};

static int __maybe_unused memwl_usb_product_id_rndis[] = {
	0x076E, /* Internet Sharing: rndis+mtp+adb+ums+diag */
	0x0774, /* IPT: rndis+mtp+adb+ums+diag */
	0x076F, /* Internet Sharing: rndis+mtp+ums+diag */
	0x0775, /* IPT: rndis+mtp+ums+diag */
	0x07CC, /* Internet Sharing: rndis+mtp+adb+ums+acm*/
	0x07D0, /* IPT: rndis+mtp+adb+ums+acm */
	0x07CD, /* Internet Sharing: rndis+mtp+ums+acm */
	0x07D1, /* IPT: rndis+mtp+ums+acm */
};
static int __maybe_unused memwl_usb_product_id_match(int product_id, int intrsharing)
{
	...
	switch (product_id) {
		case 0x0f8c: /* rndis+mtp+adb+ums+diag */
			category = 0;
			break;
		case 0x0f8d: /* rndis+mtp+ums+diag*/
			category = 1;
			break;
...
static int __maybe_unused a3cl_usb_product_id_match_array[] = {
	0x0ff8, 0x07e0, /* CDC-ECM */
	0x0fa4, 0x07e3, /* mtp+adb+ums+rmnet */
	0x0fa5, 0x07e4, /* mtp+ums+rmnet */
	0x0f91, 0x07e7, /* mtp+ums */
	0x0f64, 0x07fe, /* mtp+ums+acm */
	0x0f63, 0x07ff, /* mtp+adb+ums+acm*/
	0x0f29, 0x07fc, /* mtp+adb+ums+acm+rmnet */
	0x0f2a, 0x07fd, /* mtp+ums+acm+rmnet */
	0x0f9a, 0x07e6, /* mtp+ums+diag */
	0x0f99, 0x07e5, /* mtp+ums+adb+diag */
	-1,
};

static int __maybe_unused a3cl_usb_product_id_rndis[] = {
	0x07ea, /* Internet Sharing: rndis+mtp+adb+ums+diag */
	0x07f0, /* IPT: rndis+mtp+adb+ums+diag */
	0x07eb, /* Internet Sharing: rndis+mtp+ums+diag */
	0x07f1, /* IPT: rndis+mtp+ums+diag */
	0x07f4, /* Internet Sharing: rndis+mtp+adb+ums+acm*/
	0x07f8, /* IPT: rndis+mtp+adb+ums+acm */
	0x07f5, /* Internet Sharing: rndis+mtp+ums+acm */
	0x07f9, /* IPT: rndis+mtp+ums+acm */
};
...
/*8x26_FIXME to remove icharger and igague before finish porting*/
	.guage_driver = 0,
