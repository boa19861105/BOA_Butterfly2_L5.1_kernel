...
static void dwc3_msm_ss_phy_reg_init(struct dwc3_msm *mdwc)
{
	...
	/* WORKAROUND:
	It's possible to trigger ramdump while plugging AC in/out.
	Our flow will block on this function but we didn't use this function on this project.
	So we return this function here.
	*/
	return;
...
static int __devinit dwc3_msm_probe(struct platform_device *pdev)
{
	...
	/* we use HTC style accessory detection, so disable QCT ISR*/
	if (0/*msm->ext_xceiv.otg_capability*/) {
		mdwc->pmic_id_irq =
