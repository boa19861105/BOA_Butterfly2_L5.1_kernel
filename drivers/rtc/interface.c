...
void screenoff_policy_change(void) {
	...
	if (screenoff_policy == SCREENOFF_POLICY_15) {
		...
		screen_off_sec3 = screen_off_sec4 + 15;
		screen_off_sec2 = screen_off_sec4 + 30;
		screen_off_sec1 = screen_off_sec4 + 45;
		...
		screen_off_sec1 = screen_off_sec2 + 30;
...
static int __rtc_set_alarm(struct rtc_device *rtc, struct rtc_wkalrm *alarm)
{
	...
	if (!rtc->ops)
		 ...
		 * 0: disable
		 * 1: enable
		 * If it is EPS/nightmode, the unit of rtc_alarm will change from sec to min for powersave.
		 */
		...
		} else if ( (powersave_enabled == 1) || (powersave_enabled == 2) || (nightmode_enabled == 1)) {
			/*
			 * alarm->time.tm_min == 60 is safe
			 * because set_alarm does not check and mktime convert to seconds correctly.
			 */
